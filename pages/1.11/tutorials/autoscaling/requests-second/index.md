...
menuWeight: 1
excerpt: Learn to use the Marathon LB autoscale application
...
The marathon-lb-autoscale application collects data from all HAProxy instances to determine the current RPS (requests per second) for your apps. The autoscale controller then attempts to maintain a defined target number of requests per second per service instance. marathon-lb-autoscale makes API calls to Marathon to scale the app.

For more information, see the Marathon-LB advanced features [documentation](/1.11/networking/marathon-lb/advanced/).
